...
function ChipIcon() {
  return (
    <svg width="38" height="28" viewBox="0 0 36 26" fill="none" xmlns="http://www.w3.org/2000/svg">
      <rect width="36" height="26" rx="4" fill="#E8C66A"/>
      <path d="M0 9H11C12.1046 9 13 9.89543 13 11V15C13 16.1046 12.1046 17 11 17H0" stroke="#CFA948" strokeWidth="1.5"/>
      <path d="M36 9H25C23.8954 9 23 9.89543 23 11V15C23 16.1046 23.8954 17 25 17H36" stroke="#CFA948" strokeWidth="1.5"/>
...
function ContactlessIcon({ variant }: { variant: CardVariant }) {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" className={variant === "lime" ? "text-ink" : "text-white"}>
...
function DecorativeRings({ variant }: { variant: CardVariant }) {
  ...
  return (
    <svg
      ...
      viewBox="0 0 160 200"
    >
      <circle cx="120" cy="100" r="85" stroke={color} strokeWidth="30" fill="none" />
      <circle cx="120" cy="100" r="50" stroke={color} strokeWidth="18" fill="none" />
    </svg>
  );
}
    ...
    >
      ...
      <div className="relative z-10 mt-auto w-full">
        ...
        <div className="flex items-end justify-between">
          <div>
            <p className={`text-sm font-semibold uppercase tracking-wider ${isLime ? "text-ink" : "text-white"}`}>
              {card.name}
            </p>
            <p className={`mt-0.5 text-xs font-medium ${isLime ? "text-ink/70" : "text-white/70"}`}>
              EXP {card.expiry}
            </p>
          </div>
          <span
